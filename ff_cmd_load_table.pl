...
######################################################################
#
# File      : ff_cmd_load_table.pl
#
# Author    : Barry Kimelman
#
# Created   : July 29, 2005
#
# Purpose   : Perl/Tk script to implement the "Load Table" command.
#
######################################################################
...
use My::Myglobalvars qw($mainwin @session_command_history $window_background_color $frame_background_color
			...
			$errmsg %valid_comparison_operators %data_quotes $dialog_font);
...
######################################################################
#
# Function  : generate_load_table_window
#
...
sub generate_load_table_window
{
	my ( $count );

	$load_win2 = $mainwin->Toplevel;
	$load_win2->grab;
	$query_data_flag = 0;
	$table_selected_flag = 0;
	$num_selected_columns = 0;
	push @session_command_history,"load table";

###	$load_win2->minsize(qw(60 25));
	$load_win2->title("Load Table");
	$load_win2->configure(-background=>$window_background_color);

	my $mainframe = $load_win2->Frame(-background => $frame_background_color,)->pack(-side => 'top',
						-fill => 'x');
	...
	$table_label = $leftframe_1->Label(-text => 'Select table to be loaded',
	...
	$select_table_listbox->bind('<1>' => \&event_load_table_selection);
...
###	$query_result_textbox->configure(-wrap => 'word', -wrapLength => 0);
	...
	return;
} # end of generate_load_table_window

######################################################################
#
# Function  : exit_load_window
#
...
sub exit_load_window
{
	$load_win2->grabRelease;
	$load_win2->destroy;

	return;
} # end of exit_load_window
...
sub execute_load
{
	...
	unless ( $table_selected_flag ) {
		$load_win2->messageBox(-type => 'OK', -icon => 'error',
...
#
# Purpose   : Process a click of the "Print" button on the SELECT
#             window.
#
# Inputs    : (none)
#
# Output    : Print the query result on the system default printer.
#
# Returns   : (nothing)
#
# Example   :
...
sub print_load_data
{
	my ( $query_data , $criteria , $columns , $tablename , $query_result );

	if ( $query_data_flag ) {
		$query_data = $query_result_textbox->get('1.0','end');
		$columns = $selected_columns;
		$query_result = "SELECT $columns FROM $selected_table";
		$criteria = $select_criteria_textbox->get('1.0','end');
		chomp $criteria;
		if ( 0 < length $criteria ) {
			$query_result .=  " WHERE [$criteria]";
		} # IF
		$query_result .= "\n\n$query_data";
		...
		$load_win2->messageBox(-type => 'OK', -icon => 'error',
						-message => "There is no query result." ,
						-title => 'Query Data' );
	} # ELSE

	return;
} # end of print_load_data

######################################################################
#
# Function  : display_load_table_help_text
#
...
#
# Example   : display_load_table_help_text();
#
# Notes     : (none)
#
######################################################################

sub display_load_table_help_text
{
...
ENDHELP
	...
	$dialog = $load_win2->Dialog(-text => $message, -bitmap => 'info',
			-title => 'Load Table Help', -default_button => 'Continue',
			-buttons => [qw/Continue/] , -wraplength => '6i' ,
			-font => $dialog_font );
	$dialog->Show();

	return;
} # end of display_load_table_help_text

######################################################################
#
# Function  : event_load_table_selection
#
...
# Returns   : If success Then 1 Else 0
#
# Example   : $status = event_load_table_selection();
#
# Notes     : (none)
#
######################################################################

sub event_load_table_selection
{
	...
	$select_table_listbox->bind('<1>' => \&event_load_column_selection);

	return 1;
} # end of event_load_table_selection

######################################################################
#
# Function  : event_load_column_selection
#
...
# Returns   : If success Then 1 Else 0
#
# Example   : $status = event_load_column_selection();
#
# Notes     : (none)
#
######################################################################

sub event_load_column_selection
{
	my ( @selected , $colname );
	...
	return;
} # end of event_load_column_selection
